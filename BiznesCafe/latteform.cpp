...
using namespace std;
LatteForm::LatteForm(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::LatteForm)
{
    ui->setupUi(this);
    connect(ui->pushButton, &QPushButton::clicked, this, &LatteForm::on_pushButton_clicked);
    installEventFilter(this);}

void LatteForm::showEvent(QShowEvent *event)
{
...
void LatteForm::on_Buttpl_clicked()
{
...
void LatteForm::on_Buttmin_clicked()
{
    int* itemPrice = new int(ui->numberqwert->text().toInt());
    (*itemPrice)--;
    if (*itemPrice >= 0)
...
void LatteForm::on_pushButton_clicked()
{
...
void LatteForm::on_Zerno_clicked()
{
    ui->numberqwert->setText(QString::number(0));
    // Check if a MainWindow instance already exists
    QList<QWidget*> topLevelWidgets = QApplication::topLevelWidgets();
