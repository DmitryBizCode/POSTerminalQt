...
using namespace std;
PiesForm::PiesForm(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::PiesForm)
{
    ui->setupUi(this);
    connect(ui->pushButton, &QPushButton::clicked, this, &PiesForm::on_pushButton_clicked);
    installEventFilter(this);}

void PiesForm::showEvent(QShowEvent *event)
{
...
void PiesForm::on_Buttpl_clicked()
{
...
void PiesForm::on_Buttmin_clicked()
{
    int* itemPrice = new int(ui->numberqwert->text().toInt());
    (*itemPrice)--;
    if (*itemPrice >= 0)
...
void PiesForm::on_pushButton_clicked()
{
...
void PiesForm::on_Zerno_clicked()
{
    ui->numberqwert->setText(QString::number(0));
    // Check if a MainWindow instance already exists
    QList<QWidget*> topLevelWidgets = QApplication::topLevelWidgets();
