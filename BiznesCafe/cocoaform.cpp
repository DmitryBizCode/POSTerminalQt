...
using namespace std;
CocoaForm::CocoaForm(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CocoaForm)
{
    ui->setupUi(this);
    connect(ui->pushButton, &QPushButton::clicked, this, &CocoaForm::on_pushButton_clicked);
    installEventFilter(this);}

void CocoaForm::showEvent(QShowEvent *event)
{
...
void CocoaForm::on_Buttpl_clicked()
{
...
void CocoaForm::on_Buttmin_clicked()
{
    int* itemPrice = new int(ui->numberqwert->text().toInt());
    (*itemPrice)--;
    if (*itemPrice >= 0)
...
void CocoaForm::on_pushButton_clicked()
{
...
void CocoaForm::on_Zerno_clicked()
{
    ui->numberqwert->setText(QString::number(0));
    // Check if a MainWindow instance already exists
    QList<QWidget*> topLevelWidgets = QApplication::topLevelWidgets();
